...
void Turtlebot3Drive::update_callback()
{
  ...
  const double DIST_THRESHOLD_FRONT = 0.5; // Distance threshold for front wall
  const double DIST_THRESHOLD_LEFT = 0.5;  // Distance threshold for left wall
  const double DIST_THRESHOLD_RIGHT = 0.5; // Distance threshold for right wall
  const double LINEAR_SPEED = 0.2;         // Linear speed
  const double TURN_SPEED = 1.0;           // Turning speed for sharper turns
  const double TURN_SPEED_SLOW = 0.4;      // Slower turning speed for wider turns
  //const double TURN_ANGLE = 0.2;           // Angle adjustment to handle turning
  ...
  if (left_distance < DIST_THRESHOLD_LEFT) {
    ...
    // Wall on the right, path is clear, move forward with slight left turn
    linear_speed = LINEAR_SPEED;
    angular_speed = TURN_SPEED_SLOW; // Slight turn left to follow the wall
  } else {
    // No significant obstacles detected, maintain forward movement with slight left turn
    linear_speed = LINEAR_SPEED;
