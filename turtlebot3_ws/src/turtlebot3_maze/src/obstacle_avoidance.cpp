...
: Node("obstacle_avoidance") 
{
  ...
  "sensor_data", qos, std::bind(&ObstacleAvoidance::SensorDataCallback, this, std::placeholders::_1));

  // Subscription to robot pose yaw
  yaw_sub = this->create_subscription<std_msgs::msg::Float64>(
  "robot_pose_yaw", qos, std::bind(&ObstacleAvoidance::RobotPoseCallback, this, std::placeholders::_1));

  // Subscription to green color percentage
  green_percentage_sub = this->create_subscription<std_msgs::msg::Float64>(
  "green_color_percentage", qos, std::bind(&ObstacleAvoidance::GreenPercentageCallback, this, std::placeholders::_1));
  
  // Subscription to green goal position
  green_goal_position_sub = this->create_subscription<std_msgs::msg::Float64MultiArray>("green_goal_position", rclcpp::QoS(10), 
  std::bind(&ObstacleAvoidance::GreenGoalPositionCallback, this, std::placeholders::_1));
...
void ObstacleAvoidance::GreenPercentageCallback(const std_msgs::msg::Float64::SharedPtr msg) 
{
  // Update the percentage of green detected
  green_percentage = msg->data;
}

void ObstacleAvoidance::GreenGoalPositionCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) 
{
   // Update the x and y coordinates of the green goal position if valid data is received
  if (msg->data.size() == 2) 
  {
...
void ObstacleAvoidance::SensorDataCallback(const std_msgs::msg::Float64MultiArray::SharedPtr msg) 
{
...
void ObstacleAvoidance::RobotPoseCallback(const std_msgs::msg::Float64::SharedPtr msg) 
{
  ...
  auto cmd_vel = velocity_commander.GenerateVelocityCommand(front_dist, left_dist, right_dist, green_percentage, green_goal_x);
